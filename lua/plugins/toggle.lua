...
local toggleterm = {
  ...
  keys = {
    {
      "<A-t>",
      "<cmd>1ToggleTerm direction=horizontal size=10 name=Horizontal<cr>",
      desc = "Toggle horizontal [t]erminal",
      mode = { "n", "t" },
    },
    {
      "<A-v>",
      "<cmd>2ToggleTerm direction=vertical size=50 name=Vertical<cr>",
...
local lazygit = {
  ...
  keys = { { "<leader>g", "<cmd>LazyGitCurrentFile<cr>", desc = "LazyGit" } },
}
...
local nvim_tree = {
  ...
  keys = { { "<leader>e", "<cmd>NvimTreeFindFileToggle<cr>", desc = "Toggl[e] nvim tree" } }
}
...
return { telescope, toggleterm, lazygit, nvim_tree, undotree }
